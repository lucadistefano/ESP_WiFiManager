...
boolean ESP_WiFiManager::autoConnect(char const *apName, char const *apPassword)
{
    ...
    uint8_t status = waitForConnectResultWithTimeout();
    unsigned long startedAt = millis();
    
    if (status == WL_CONNECTED)
    {
        float waited = (millis() - startedAt);
        DEBUG_WM(F("After waiting "), false);
        DEBUG_WM(waited / 1000, false);
        DEBUG_WM(F(" secs, local ip: "), false);
...
uint8_t ESP_WiFiManager::waitForConnectResult()
{
    ...
    {    
        return waitForConnectResultWithTimeout();
    }
}

uint8_t ESP_WiFiManager::waitForConnectResultWithTimeout()
{       
...
void ESP_WiFiManager::DEBUG_WM(Generic text, bool nl)
{
    if (_debug)
    {
        Serial.print("*WM: ");
